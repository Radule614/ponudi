...
import { combineLatest, tap, Observable } from "rxjs";
...
export class CategoryComponent extends UnsubscribeComponent implements OnInit {
  ...
  categoryParams$: Observable<any> = combineLatest([this.route.params, this.route.queryParams, this.store.select(CategorySelectors.selectAll)]).pipe(
    tap(([params, queryParams, categories]) => {
